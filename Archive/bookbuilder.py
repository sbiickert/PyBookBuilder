...
class BookInfoForm(npyscreen.ActionForm):
    ...
    def beforeEditing(self):
        ...
        all_characters = ", ".join(book.characters)
...
class BookContentList(npyscreen.MultiLineAction):
    ...
    def display_value(self, vl):
        ...
        if isinstance(vl, Scene):
            ...
            variable_width = self.width - (padding_and_other + 8 + 9)
            pov = fit_str(scene.pov, 9) # Fixed width
            loc = fit_str(scene.location, round(variable_width * 0.4))
            desc = fit_str(scene.description, round(variable_width * 0.6)) 
            w_count = f'({scene.word_count()})'.rjust(8) # Fixed width
...
class ChapterForm(npyscreen.ActionPopupWide):
    ...
    def beforeEditing(self):
        ...
        if chapter is not None:
            ...
            self.characters_text.value = ", ".join(chapter.get_characters())
...
class SceneForm(npyscreen.ActionForm):
    ...
    def beforeEditing(self):
        ...
        if scene is not None:
            ...
            self.characters_text.value = ', '.join(scene.get_characters())
            ...
            characters = scene.get_chapter().get_book().characters
            idx = characters.index(scene.pov)
            self.pov_select.values = characters
