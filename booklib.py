...
TITLES = ["Lord", "Lady", "Mister", "Miss", "Mr.", "Mrs.", "Mistress", "Captain", "Sir"]

class Character():
    """ Represents a character from the book """
    def __init__(self, data: Dict[str, Any] = None):
        """
        Initializes the :class:`booklib.Character`.
        """
        self.name = ""
        self.description = ""
    ...
    def set_data(self, data_dict: Dict[str, Any]):
        ...
        :param data_dict: the character data that was parsed from JSON
        """
        try:
            self.description = data_dict['description']
            self.name = data_dict['name']
            if 'aliases' in data_dict:
                self.aliases = data_dict['aliases']
        except KeyError:
            logging.info("Missing header key.")
    
    def is_mentioned_in_text(self, text:str) -> bool:
        """
        Look for the name, or fragments thereof, or aliases
        """
        all_names = self.name.split() + self.aliases
        all_names = list(filter(lambda name: name not in TITLES, all_names))
        return any(name in text for name in all_names)
    
    def write_data(self) -> Dict[str, Any]:
        """
        Outputs the :class:`booklib.Character` ivars to a dict.
        """
        data_dict: Dict[str, Any] = {}
        data_dict['description'] = self.description
        data_dict['name'] = self.name
        data_dict['aliases'] = self.aliases
        return data_dict
...
class Scene():
    ...
    def get_pov_character(self) -> Character:
        """ Returns the Character named in the pov ivar """
        return self.get_chapter().get_book().find_character(self.pov)

    def get_characters(self) -> List[str]:
        """ Searches for known characters in the Scene """
        found_characters: List[str] = []
        the_book = self.get_chapter().get_book()
        all_characters = the_book.major_characters + the_book.minor_characters
        for character in all_characters:
            for paragraph in self.paragraphs:
                if character.is_mentioned_in_text(paragraph):
...
class Chapter():
    ...
    def get_characters(self) -> List[str]:
        ...
        uniques = list(set(all_characters))
        return uniques
...
class Book():
    ...
    def find_character(self, name: str) -> Optional[Character]:
        """ Finds a character by name or alias """
        for ch in self.major_characters + self.minor_characters:
            if ch.name == name or name in ch.aliases or name in ch.name or ch.name in name:
                return ch
        return None
    ...
    def set_header(self, header_dict: Dict[str, Any]):
        ...
        self.major_characters = []
        for ch_data in header_dict['characters']['major']:
            self.major_characters.append(Character(ch_data))
        self.minor_characters = []
        for ch_data in header_dict['characters']['minor']:
            self.minor_characters.append(Character(ch_data))
    ...
    def write_header(self) -> str:
        ...
        header_dict['characters'] = {}
        header_dict['characters']['major'] = list(map(lambda c: c.write_data(), self.major_characters))
        header_dict['characters']['minor'] = list(map(lambda c: c.write_data(), self.minor_characters))
        json_text = json.dumps(header_dict, indent=2)
        return f"<!-- {json_text} -->"
